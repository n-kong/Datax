                ...
                case Types.DATE:
                    if (this.resultSetMetaData.getRight().get(columnIndex)
                        ...
                        try {
                            ...
                            utilDate = null;
                            LOG.warn(String.format("TIMESTAMP 类型转换错误：[%s] 该字段值置空!", column));
...
//                                "TIMESTAMP 类型转换错误：[%s]", column));
                ...
                case Types.LONGVARBINARY:
                    preparedStatement.setBytes(columnIndex + 1, null == column
                            .asBytes() ? null : column.asBytes());
                    break;

                case Types.BOOLEAN:
                    preparedStatement.setString(columnIndex + 1, StringUtils.isEmpty(column.asString()) ? null : column.asString());
                    break;
                ...
                case Types.BIT:
                    if (this.dataBaseType == DataBaseType.MySql) {
                        preparedStatement.setBoolean(columnIndex + 1, null == column.asBoolean() ? null:column.asBoolean());
                    } else {
                        preparedStatement.setString(columnIndex + 1, StringUtils.isEmpty(column.asString()) ? null : column.asString());
                    }
                    break;
