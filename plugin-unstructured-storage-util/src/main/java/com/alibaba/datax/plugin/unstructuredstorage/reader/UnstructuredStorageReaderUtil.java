...
import com.alibaba.datax.plugin.unstructuredstorage.util.MD5;
import com.alibaba.datax.plugin.unstructuredstorage.util.WriteOSS;
...
public class UnstructuredStorageReaderUtil {
    ...
    private static WriteOSS writeOSS;
    ...
    public static void doReadFromStream(BufferedReader reader, String context,
        ...
        try {
            ...
            String imageIndex = readerSliceConfig.getString(Key.IMAGE_INDEX);

            String oss_prefix = "";
            String oss_dir = "";
            if (null != imageIndex) {
                writeOSS = new WriteOSS(readerSliceConfig);
                String pre = readerSliceConfig.getString(Key.OSS_PREFIX);
                oss_prefix = pre.endsWith("/") ? pre : pre + "/";
                oss_dir = readerSliceConfig.getString(Key.DIR_NAME);
            }
            boolean flg = true;
            int columnNumber = 0;
            while ((parseRows = UnstructuredStorageReaderUtil
                ...
                if (flg) {
                    flg = false;
                    columnNumber = parseRows.length;
                }
                if (columnNumber != parseRows.length) {
                    LOG.error(String.format("首次读取文件[%s]的列数为[%s]，本次读取的列数为[%s]，数据为[%s]", context, columnNumber, parseRows.length, StringUtils.join(parseRows, "\t")));
                    continue;
                    //throw DataXException.asDataXException(
                    //    UnstructuredStorageReaderErrorCode.COLUMN_NUMBER_EXCEPTION,
                    //    String.format("首次读取文件[%s]的列数为[%s]，本次读取的列数为[%s]，数据为[%s]", context, columnNumber, parseRows.length, StringUtils.join(parseRows, "\t")));
                }
                dealRowIfImage(parseRows, oss_prefix, oss_dir, imageIndex);
    ...
    private static void dealRowIfImage(String[] parseRows, String oss_prefix, String oss_dir,
        String imageIndex) {

        if (null != imageIndex) {
            String[] ids = imageIndex.split(",", -1);
            for (String id : ids) {
                String imageValue = parseRows[Integer.parseInt(id)];
                if (StringUtils.isNotEmpty(imageValue)) {
                    String key = oss_dir + "/" + MD5.encrypt2(imageValue);
                    byte[] bytes = Base64.decodeBase64(imageValue);
                    writeOSS.writeOss(key, bytes);
                    parseRows[Integer.parseInt(id)] = oss_prefix + key;
                }
            }
        }
    }
    ...
    public static Record transportOneRecord(RecordSender recordSender,
        ...
        return transportOneRecord(recordSender, column, sourceLine, nullFormat,
            taskPluginCollector);
    }
    ...
    public static Record transportOneRecord(RecordSender recordSender,
        ...
        if (null == columnConfigs || columnConfigs.size() == 0) {
            for (String columnValue : sourceLine) {
                ...
                if (columnValue.equalsIgnoreCase(nullFormat)) {
    ...
    public static void validateEncoding(Configuration readerConfiguration) {
        ...
        try {
            ...
            throw DataXException
                .asDataXException(UnstructuredStorageReaderErrorCode.CONFIG_INVALID_EXCEPTION,
    ...
    public static void validateCompress(Configuration readerConfiguration) {
        String compress = readerConfiguration
            .getUnnecessaryValue(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COMPRESS,
                null, null);
        if (StringUtils.isNotBlank(compress)) {
            compress = compress.toLowerCase().trim();
            boolean compressTag =
                "gzip".equals(compress) || "bzip2".equals(compress) || "zip".equals(compress)
                    || "lzo".equals(compress) || "lzo_deflate".equals(compress) ||
                    "hadoop-snappy".equals(compress)
                    || "framing-snappy".equals(compress);
            if (!compressTag) {
                throw DataXException
                    .asDataXException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
                        String.format(
                            "仅支持 gzip, bzip2, zip, lzo, lzo_deflate, hadoop-snappy, framing-snappy " +
                                "文件压缩格式, 不支持您配置的文件压缩格式: [%s]", compress));
            }
        ...
        readerConfiguration
            .set(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COMPRESS, compress);
    }
    ...
    public static void validateColumn(Configuration readerConfiguration) {
        ...
        if (null != columns && 1 == columns.size()) {
            ...
            if ("\"*\"".equals(columnsInStr) || "'*'".equals(columnsInStr)) {
                readerConfiguration
                    .set(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COLUMN, null);
                columns = null;
            }
        }
        ...
        if (null != columns && columns.size() != 0) {
            for (Configuration eachColumnConf : columns) {
                eachColumnConf
                    .getNecessaryValue(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.TYPE,
                ...
                if (null == columnIndex && null == columnValue) {
                    throw DataXException
                        .asDataXException(UnstructuredStorageReaderErrorCode.NO_INDEX_VALUE,
                            "由于您配置了type, 则至少需要配置 index 或 value");
                }

                if (null != columnIndex && null != columnValue) {
                    throw DataXException
                        .asDataXException(UnstructuredStorageReaderErrorCode.MIXED_INDEX_VALUE,
                            "您混合配置了index, value, 每一列同时仅能选择其中一种");
                }
        ...
        if (StringUtils.isNotBlank(csvReaderConfig)) {
            try {
                UnstructuredStorageReaderUtil.csvReaderConfigMap =
                    JSON.parseObject(csvReaderConfig, new TypeReference<HashMap<String, Object>>() {
                    });
            } catch (Exception e) {
                LOG.info(String.format("WARN!!!!忽略csvReaderConfig配置! 配置错误,值只能为空或者为Map结构,您配置的值为: %s",
                    csvReaderConfig));
            }
        }
    }
    ...
    public static void setCsvReaderConfig(CsvReader csvReader) {
        if (null != UnstructuredStorageReaderUtil.csvReaderConfigMap &&
            !UnstructuredStorageReaderUtil.csvReaderConfigMap.isEmpty()) {
            try {
                BeanUtils.populate(csvReader, UnstructuredStorageReaderUtil.csvReaderConfigMap);
                LOG.info(String
                    .format("csvReaderConfig设置成功,设置后CsvReader:%s", JSON.toJSONString(csvReader)));
            } catch (Exception e) {
                LOG.info(String.format(
                    "WARN!!!!忽略csvReaderConfig配置!通过BeanUtils.populate配置您的csvReaderConfig发生异常,您配置的值为: %s;请检查您的配置!CsvReader使用默认值[%s]",
            ...
            csvReader.setUseTextQualifier(false);
            LOG.info(String
                .format("CsvReader使用默认值[%s],csvReaderConfig值为[%s]", JSON.toJSONString(csvReader),
                    JSON.toJSONString(UnstructuredStorageReaderUtil.csvReaderConfigMap)));
        }
    }
